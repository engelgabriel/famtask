...
// When adding tag to a reward, ID of the reward
Session.setDefault('editing_addtag', null);

// When editing a list name, ID of the list
Session.setDefault('editing_listname', null);

// When editing reward text, ID of the reward
...
// Always be subscribed to the rewards for the selected list.
...
Template.rewards.rewards = function () {
  // Determine which rewards to display in main pane,
...
Template.reward_item.events({
  ...
  'dblclick .display .reward-text': function (evt, tmpl) {
    Session.set('editing_itemname', this._id);
    Deps.flush(); // update DOM before focus
    activateInput(tmpl.find("#reward-input"));
  },
...
// Pick out the unique tags from all rewards in current list.
Template.tag_filter.tags = function () {
  var tag_infos = [];
  var total_count = 0;

  Rewards.find({list_id: Session.get('list_id')}).forEach(function (reward) {
    _.each(reward.tags, function (tag) {
...
var RewardsRouter = Backbone.Router.extend({
  routes: {
    ":list_id": "main"
  },
